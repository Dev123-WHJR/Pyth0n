...
def myCardDetails():
    ...
    pin = "454584"
    print(type(pin))
    address = "Walt Disney Animation Studios"
    print(type(address))
    auth = "Two wheeler, Four Wheeler"
    print(type(auth))
    ...
    label_authorization['text'] = auth
# Create a button
button1 = Button(root, text = "Show my Driving Liscense", command=myCardDetails )
...
label_dob_window = canvas.create_window(140, 120, anchor=CENTER, window=label_birth)
...
label_vehicle_no_window = canvas.create_window(335, 180, anchor=CENTER, window=label_authorization)
canvas.pack()
